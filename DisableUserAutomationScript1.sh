...
echo "mv alias mail.txt" >> filename
chmod +x filename
ARGUMENTCOMMAND="sudo -S ./filename"


list=`${USERLIST_COMMAND}`
SSH_USERNAME="sushmita"
...
do
    if [ "`ping -c 1 ${IP}`" ]
        then
            ...
            scp filename ${SSH_USERNAME}@${IP}:/home/${SSH_USERNAME}
            AliaseRemovalCommand="ssh -p 22 ${SSH_USERNAME}@${IP} ${ARGUMENTCOMMAND}"
