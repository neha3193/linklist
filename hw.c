...
static int init_filters(const char *filters_descr)
{
	...
	snprintf(args, sizeof(args),
			"video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d",
			dec_ctx->width, dec_ctx->height, 23,
			time_base.num, time_base.den,
			dec_ctx->sample_aspect_ratio.num, dec_ctx->sample_aspect_ratio.den);
...
int hardware( )
{
	...
	/*Socket creation to send filters messeges on UDP socket*/
        ...
        servaddr.sin_port = htons(atoi(getvalue("UDP_PORT")));
        servaddr.sin_addr.s_addr = INADDR_ANY;
	...
	filter_descr = (char *)malloc(100 * sizeof(char));
    //char value[100];
    //strcpy(value, getvalue("BlackDetect"));
    printf("get value blackdetect %s \n", getvalue("BlackDetect"));
    //printf("get value blackdetect %s \n", value);
    printf("get value FreezeDetect %s \n", getvalue("FreezeDetect"));
	if(!(strcasecmp(getvalue("BlackDetect"), "ON")) && !(strcasecmp(getvalue("FreezeDetect"), "ON")))
	{
		sprintf(filter_descr, 
				"select='not(mod(n,5)'),blackdetect=d=%s:pix_th=%s,freezedetect=n=-%sdB:d=%s",
				getvalue("blackDuration"), getvalue("blackThreshold"),
				getvalue("freezeNoise"), getvalue("freezeDurationlue"));
	//	printf("filter_descr: %s \n",filter_descr);
	}
	else if(!(strcasecmp(getvalue("BlackDetect"), "ON")))
	{
		sprintf(filter_descr, "select='not(mod(n,5)'),blackdetect=d=%s:pix_th=%s", getvalue("blackDuration"), getvalue("blackThreshold"));
	}
	else if(!(strcasecmp(getvalue("FreezeDetect"), "ON")))
	{
		sprintf(filter_descr, "select='not(mod(n,5)'),freezedetect=n=-%sdB:d=%s", getvalue("freezeNoise"), getvalue("freezeDuration"));
	}

//strcpy(filter_descr,"blackdetect=d=1.5:pix_th=0.10:pic_th=0.70");
	filter_audio_descr = (char *)malloc(100 * sizeof(char));
	if(!(strcasecmp(getvalue("SilenceDetect"), "ON")))
	{
		sprintf(filter_audio_descr, "silencedetect=n=-%sdB:d=%s", getvalue("silenceNoise"), getvalue("silenceDuration"));
	...
	if(!(strcasecmp(getvalue("HARDWARE"),"ON")))
	{
	...
	sprintf(Multicast_SRC, "udp://%s:%s", getvalue("MULTICAST_IP"),getvalue("MULTICAST_PORT"));
	...
	ret = av_find_best_stream(fmt_ctx, AVMEDIA_TYPE_VIDEO, videopid, -1, &dec, 0);
	...
	// if (avcodec_parameters_to_context(dec_ctx, video->codecpar) < 0)
...
//			goto end;
	...
	if(!(strcasecmp(getvalue("SilenceDetect"), "ON")))
        {
		if(filter_audio_descr != NULL)
		{
			if ((ret = init_audio_filters(filter_audio_descr)) < 0)
				goto end;
		}
	}   
		...
		if((!(strcasecmp(getvalue("BlackDetect"), "ON")) || (!(strcasecmp(getvalue("FreezeDetect"), "ON")) )))
		{
			...
			if (packet.stream_index == video_stream_index)
			{
				...
				while (ret >= 0)
				{
					...
					tmp_frame->pts = frame->best_effort_timestamp;
					...
					while (1)
					{
						...
						if (ret < 0)
							goto end;
						                        /*Code for demo review*/
                        if ((tag_ratio = av_dict_get(filt_frame->metadata,"lavfi.picture_black_ratio", tag_ratio, AV_DICT_IGNORE_SUFFIX)))
                        {   
                        ...
                        blackTag = NULL;
                        blackTag = av_dict_get(filt_frame->metadata, "lavfi.black_start", blackTag, AV_DICT_IGNORE_SUFFIX);
                        ...
                        blackTag1 = NULL;
                        blackTag1 = av_dict_get(filt_frame->metadata, "lavfi.black_end", blackTag1, AV_DICT_IGNORE_SUFFIX);
                        ...
                        freezeTag = NULL;
                        freezeTag = av_dict_get(filt_frame->metadata, "lavfi.freezedetect.freeze_start", freezeTag, AV_DICT_IGNORE_SUFFIX);
                        ...
                        freezeTag1 = NULL;
                        freezeTag1 = av_dict_get(filt_frame->metadata, "lavfi.freezedetect.freeze_end", freezeTag1, AV_DICT_IGNORE_SUFFIX);
