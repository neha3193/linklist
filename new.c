...
int main(int argc, char *argv[])
{
        ...
        if (pid < MAX_PID)
        {
            ...
            if (pid == 0x11)
            {
                if (table_id == 0x42)
                {
                    ...
                    if (reports)
                        fprintf(sdt_delta_values, "%.3f\n", value);
                    sdt_count++;
                    last_sdt = position;
                }
                else if (table_id == 0x4A)
                { // table_id== 0x46 |||| table_id== 0x72
            ...
            if (pid == 0x14)
            {
                if (table_id == 0x70)
                {
                    ...
                    if (reports)
                        fprintf(tdt_delta_values, "%.3f\n", value);
                    tdt_count++;
                    last_tdt = position;
                }
                else if (table_id == 0x73)
                { // 72???|| table_id== 0x72) //ST TABLE
                    if (check_crc(packet) != 0)
                    {
                ...
                if (pid == pcr_pids[i])
                {
                    ...
                    if ((packet[3] & 0x20) && (packet[4] != 0) && (packet[5] & 0x10))
                    {
                        ...
                        else
                        {
                            ...
                            if (pcr_jitter > 0.05)
                            { // is this right? the whole nano second stuff seems to be throwing me for a loop
    ...
    */
    ...
    for (i = 0; i < pid_count; i++)
    {
        ...
        for (b = 0; b < streams; b++)
        {
            if (total_pids[i] == streampids[b])
                match = 1;
        }
        if (!match)
            fprintf(stdout, "3.4  - ERROR - PID_error - PID (%d) found in stream, not in PMT\n", total_pids[i]);
    }
    if (last_nit == 0)
    {
